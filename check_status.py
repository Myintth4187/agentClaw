#!/usr/bin/env python3
"""frameClaw platform health check script.
...
def check_docker_containers() -> dict[str, dict]:
    """Check status of all frameclaw-related Docker containers."""
    ok, out = run_cmd([
        "docker", "ps", "-a",
...
def check_postgres() -> tuple[bool, str]:
    ...
    ok, out = run_cmd([
        ...
        "pg_isready", "-U", "frameclaw", "-d", "frameclaw_platform",
    ])
    return ok, out
...
def check_user_containers() -> list[dict]:
    """Find and check running frameclaw user containers."""
    ok, out = run_cmd([
        "docker", "ps",
...
def main():
    parser = argparse.ArgumentParser(description="frameClaw platform health check")
    ...
    all_ok = True
    print("=== frameClaw Health Check ===\n")

    # 1. Docker containers
    print("Docker Containers:")
    containers = check_docker_containers()
    if not containers:
        print(f"  {check_mark(False)} No frameclaw containers found")
        all_ok = False
    else:
